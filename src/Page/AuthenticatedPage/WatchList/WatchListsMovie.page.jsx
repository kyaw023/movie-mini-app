...
const WatchListsMoviePage = () => {
  const { data: favorite } = useGetFavoriteQuery("movies");

  const { data: watchlist, isLoading } = useGetWatchListQuery("movies");

  const { data: ratingList } = useGetRatingQuery("movies");

  // get favorite movies in watchlist
  const getCommonItems = (favorite, watchlist) => {
    return watchlist?.filter((wl) => {
      return favorite?.some((fav) => fav?.id === wl?.id);
    });
  };
                        ...
                        if (commonItem) {
                          return (
                            <div className="flex items-center gap-2">
                              <div className=" flex items-center justify-center md:w-10 md:h-10 w-6 h-6 text-white border border-secondary-50 md:p-3 p-1 rounded-full bg-blue-600">
                          ...
                          return (
                            <div className="flex items-center gap-2">
                              <Star className="md:w-10 w-10 h-10 text-white border border-secondary-50 p-3 rounded-full" />
                              <span className="text-secondary-50">
                                Rate it!
                              </span>
                            </div>
...
export default WatchListsMoviePage;
